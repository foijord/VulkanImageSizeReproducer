...
int main(int, char* [])
{
	...
	for (auto physicalDevice : physicalDevices) {
		...
		VkExtent3D extent{
			std::min(1024u, imageFormatProperties.maxExtent.width),
			std::min(1024u, imageFormatProperties.maxExtent.height),
			std::min(1024u, imageFormatProperties.maxExtent.depth)
		};
		...
		for (size_t count = 0; count < 1000; count++) {
			...
			if (result == VK_SUCCESS) {
				...
				if (result == VK_ERROR_OUT_OF_DEVICE_MEMORY) {
					...
					std::cerr << "vkCreateImage failed with result " << result << std::endl;
				}
				break;
			}
		}
